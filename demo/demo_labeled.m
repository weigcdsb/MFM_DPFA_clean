...
for g= 2:ng
    ...
    for j = 1:nClus
        ...
        prior.x0 = zeros(THETA{g-1}(j).p,1);
        prior.Q0 = eye(THETA{g-1}(j).p);
        prior.muC0 = zeros(THETA{g-1}(j).p,1);
        prior.SigC0 = eye(THETA{g-1}(j).p);
        
        [THETA{g}(j), acc] = update_clusParam_PG(THETA{g-1}(j),...
                delt_fit(obsIdx,g-1), C_fit(obsIdx,1:THETA{g-1}(j).p,g-1),...
                Y(obsIdx,:),R_all(obsIdx,:),prior);
        ACC_trace(j,g) = acc;
        if(g > burnIn)
